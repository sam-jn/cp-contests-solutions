...
// string unmatch(string s, string t, int i){
//     int flag = 0;
//     int n = s.size();
...
void solve(){
    ...
    for(int i = 0; i <= n - 7; i++){
        ...
        if(b1.size() > 0){
            string xx = s;
            xx.replace(i, 7, b1);
            if(xx.find(t, i + 1) == string::npos && xx.substr(0, i + 6).find(t, 0) == string::npos){
                flag = 1;
                s.replace(i, 7, b1);
                break;
            }
